...
<body>
	...
	<header class="header-section">
		<div class="logo">
			<img src="../img/logo.png" height="48" width="48"><!-- Logo -->
               <span class="verde" style="margin-left: 70px;">Principal</span>
		</div>
	...
	<div class="team-section spad" style="padding-bottom: 10px;">
		<div class="container">
			...
			<?php if(isset($_SESSION['errores']['actb'])) : ?>
				<h2 style="color: #2be6ab; text-align: center; font-size: 25px; margin-bottom: 20px;"><?= $_SESSION['errores']['actb'] ?></h2>
			<?php endif; ?>
			<?php if(isset($_SESSION['errores']['act'])) : ?>
				<h2 style="color: red; text-align: center; font-size: 25px; margin-bottom: 20px;"><?= $_SESSION['errores']['act'] ?></h2>
			<?php endif; ?>
			<?php if(isset($_SESSION['errores']['actualizarb'])) : ?>
				<h2 style="color: #2be6ab; text-align: center; font-size: 25px; margin-bottom: 20px;"><?= $_SESSION['errores']['actualizarb'] ?></h2>
			<?php endif; ?>
			<?php if(isset($_SESSION['errores']['actualizar'])) : ?>
				<h2 style="color: #2be6ab; text-align: center; font-size: 25px; margin-bottom: 20px;"><?= $_SESSION['errores']['actualizarb'] ?></h2>
			<?php endif; ?>
			<div class="row">
				<!-- single member -->
					...
					<?php if(obtenerdsem('LUNES', $week) >= date('Y-m-d')) : ?>
						<div class="member">
							...
							<table border="1" width="300" style="width: 100%; font-size: 30px; color: white;">
								...
								<tr>
									<td onclick="act('IDA1', <?= $idconductor ?>, <?= substr(obtenerdsem('LUNES', $week), 0, 4) ?>, <?= substr(obtenerdsem('LUNES', $week), 5, 2) ?>, <?= substr(obtenerdsem('LUNES', $week), 8, 10) ?>)">
										<?= sacarCondTrayecto($db, obtenerdsem('LUNES', $week), 'IDA1')['NOMBRE'] ?>
									</td>

									<td onclick="act('VUELTA1', <?= $idconductor ?>, <?= substr(obtenerdsem('LUNES', $week), 0, 4) ?>, <?= substr(obtenerdsem('LUNES', $week), 5, 2) ?>, <?= substr(obtenerdsem('LUNES', $week), 8, 10) ?>)">
										<?= sacarCondTrayecto($db, obtenerdsem('LUNES', $week), 'VUELTA1')['NOMBRE'] ?>
									</td>
								</tr>
								<tr>
									<td onclick="act('IDA2', <?= $idconductor ?>, <?= substr(obtenerdsem('LUNES', $week), 0, 4) ?>, <?= substr(obtenerdsem('LUNES', $week), 5, 2) ?>, <?= substr(obtenerdsem('LUNES', $week), 8, 10) ?>)">
										<?= sacarCondTrayecto($db, obtenerdsem('LUNES', $week), 'IDA2')['NOMBRE'] ?>
									</td>
									<td onclick="act('VUELTA2', <?= $idconductor ?>, <?= substr(obtenerdsem('LUNES', $week), 0, 4) ?>, <?= substr(obtenerdsem('LUNES', $week), 5, 2) ?>, <?= substr(obtenerdsem('LUNES', $week), 8, 10) ?>)">
					...
					<div class="col-sm-4">
						<div class="member">
							<h2 style="font-size: 30px;">Jueves</h2>
								...
								<tr>
									<td onclick="act('IDA1', <?= $idconductor ?>, <?= substr(obtenerdsem('JUEVES', $week), 0, 4) ?>, <?= substr(obtenerdsem('JUEVES', $week), 5, 2) ?>, <?= substr(obtenerdsem('JUEVES', $week), 8, 10) ?>)">
										<?= sacarCondTrayecto($db, obtenerdsem('JUEVES', $week), 'IDA1')['NOMBRE'] ?>
									</td>
									<td onclick="act('VUELTA1', <?= $idconductor ?>, <?= substr(obtenerdsem('JUEVES', $week), 0, 4) ?>, <?= substr(obtenerdsem('JUEVES', $week), 5, 2) ?>, <?= substr(obtenerdsem('JUEVES', $week), 8, 10) ?>)">
										<?= sacarCondTrayecto($db, obtenerdsem('JUEVES', $week), 'VUELTA1')['NOMBRE'] ?>
									</td>
								</tr>
								<tr>
									<td onclick="act('IDA2', <?= $idconductor ?>, <?= substr(obtenerdsem('JUEVES', $week), 0, 4) ?>, <?= substr(obtenerdsem('JUEVES', $week), 5, 2) ?>, <?= substr(obtenerdsem('JUEVES', $week), 8, 10) ?>)">
										<?= sacarCondTrayecto($db, obtenerdsem('JUEVES', $week), 'IDA2')['NOMBRE'] ?>
									</td>
									<td onclick="act('VUELTA2', <?= $idconductor ?>, <?= substr(obtenerdsem('JUEVES', $week), 0, 4) ?>, <?= substr(obtenerdsem('JUEVES', $week), 5, 2) ?>, <?= substr(obtenerdsem('JUEVES', $week), 8, 10) ?>)">
						...
						<div class="col-sm-4">
							<div class="member">
								...
								<table border="1" width="300" style="width: 100%; font-size: 30px; color: white;">
									...
									<tr>
										<td onclick="act('IDA1', <?= $idconductor ?>, <?= substr(obtenerdsem('VIERNES', $week), 0, 4) ?>, <?= substr(obtenerdsem('VIERNES', $week), 5, 2) ?>, <?= substr(obtenerdsem('VIERNES', $week), 8, 10) ?>)">
											<?= sacarCondTrayecto($db, obtenerdsem('VIERNES', $week), 'IDA1')['NOMBRE'] ?>
										</td>
										<td onclick="act('VUELTA1', <?= $idconductor ?>, <?= substr(obtenerdsem('VIERNES', $week), 0, 4) ?>, <?= substr(obtenerdsem('VIERNES', $week), 5, 2) ?>, <?= substr(obtenerdsem('VIERNES', $week), 8, 10) ?>)">
											<?= sacarCondTrayecto($db, obtenerdsem('VIERNES', $week), 'VUELTA1')['NOMBRE'] ?>
										</td>
									</tr>
									<tr>
										<td onclick="act('IDA2', <?= $idconductor ?>, <?= substr(obtenerdsem('VIERNES', $week), 0, 4) ?>, <?= substr(obtenerdsem('VIERNES', $week), 5, 2) ?>, <?= substr(obtenerdsem('VIERNES', $week), 8, 10) ?>)">
											<?= sacarCondTrayecto($db, obtenerdsem('VIERNES', $week), 'IDA2')['NOMBRE'] ?>
										</td>
										<td onclick="act('VUELTA2', <?= $idconductor ?>, <?= substr(obtenerdsem('VIERNES', $week), 0, 4) ?>, <?= substr(obtenerdsem('VIERNES', $week), 5, 2) ?>, <?= substr(obtenerdsem('VIERNES', $week), 8, 10) ?>)">
